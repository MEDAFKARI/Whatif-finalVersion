...
public class SFTPIntegrationConfig {
    ...
    private static String ForOutputFilesLastName = "";
    private static int PartNUMBER = 2;
        ...
        return IntegrationFlow.from(sftpOutputChannel())
                .handle(message -> {
                    ...
                    try {
                        ...
                        sftpInboundFileSynchronizer().synchronizeToLocalDirectory(new File("output/"));

                        sftpMessageSource().receive();
...
//    }
    ...
    public MessageSource<File> sftpMessageSource() {
        ...
        source.setLocalFilter(new FileListFilter<File>() {
            @Override
            public List<File> filterFiles(File[] files) {
                return Arrays.stream(files)
                        .filter(file -> file.getName().startsWith("XMLEVO_"))
    ...
    @Bean("sftpInboundFlow")
    public IntegrationFlow sftpInboundFlow() {
        return IntegrationFlow.from(sftpMessageSource(), c -> c.poller(p -> p.fixedDelay(10)))
                .handle(message -> {
                            ...
                            if (file.exists()) {
                                ...
                                if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
                                    ...
                                    if(ForOutputFilesLastName.equals(timestamp)){
                                        String fileName = timestamp.concat("_Output_") + "part"+ PartNUMBER++ +".xml";
                                        renamedFile = new File(file.getParent(), fileName);

                                    }else {
                                        String fileName = timestamp.concat("_Output_") + "part1.xml";
                                        renamedFile = new File(file.getParent(), fileName);
                                        ForOutputFilesLastName = timestamp;
                                        PartNUMBER =2;
                                    }
